```lua
-- Загрузка библиотеки Luna Interface Suite
local Luna = loadstring(game:HttpGet("https://raw.githubusercontent.com/Nebula-Softworks/Luna-Interface-Suite/refs/heads/master/source.lua", true))()

-- Создание главного окна с логотипом
...
-- ИСПРАВЛЕННЫЙ Jump Boost — используем JumpHeight вместо JumpPower
TMovement:CreateToggle({
    Name = "Jump Boost",
    CurrentValue = false,
    Callback = function(v)
        ...
        if v then
            Connections.Jump = RunService.Heartbeat:Connect(function()
                pcall(function()
                    local hum = getHum()
                    if hum then
                        -- Используем JumpHeight (современный метод)
                        hum.JumpHeight = 7.2 * JumpMult
                        -- Запасной вариант через UseJumpPower
...
-- ИСПРАВЛЕННЫЙ Infinite Jump — используем Velocity + состояние
TMovement:CreateToggle({
    Name = "Infinite Jump",
    CurrentValue = false,
    Callback = function(v)
        disconnect("InfiniteJump")
        disconnect("InfiniteJumpRender")
        IsJumping = false

        if v then
            -- Отслеживаем нажатие Space
            ...
            Connections.InfiniteJumpRender = UserInputService.InputEnded:Connect(function(input)
                if input.KeyCode == Enum.KeyCode.Space then
                    IsJumping = false
                end
            end)

            Notify("Infinite Jump", "On — Space в воздухе")
        else
            IsJumping = false
...
-- ИСПРАВЛЕННЫЙ Mega Hitbox — фиксированный размер вместо умножения
local OriginalSizes = {}
TFeatures:CreateToggle({
    Name = "Mega Hitbox",
    CurrentValue = false,
    Callback = function(v)
        disconnect("Hitbox")
        if v then
            -- Сохраняем оригинальные размеры и применяем фиксированный множитель
            pcall(function()
...
-- Fly Mode (BodyVelocity всё ещё работает в большинстве эксплойтов)
...
-- ========== Вкладка Visuals ==========
-- ESP Players с обновлением расстояния
TVisuals:CreateToggle({
    ...
    Callback = function(v)
        ...
        if v then
            ...
            -- Обновляем расстояние в реальном времени
            Connections.ESPUpdate = RunService.Heartbeat:Connect(function()
                pcall(function()
                    local myHRP = getHRP()
                    if not myHRP then return end
                    for player, bb in pairs(ESPObjects) do
                        if player.Character and bb and bb:FindFirstChild("TextLabel") then
                            local hrp = player.Character:FindFirstChild("HumanoidRootPart")
                            if hrp then
                                bb.TextLabel.Text = player.Name .. " | " .. math.floor((hrp.Position - myHRP.Position).Magnitude) .. "m"
                            end
                        end
                    end
                end)
...
-- ========== Вкладка Protection ==========
-- ИСПРАВЛЕННЫЙ Anti Kick — через хук метатаблицы
        ...
        if v then
            pcall(function()
                ...
                if getrawmetatable and hookfunction then
                    ...
                    -- Fallback: просто замена функции (работает в некоторых старых эксплойтах)
                    LP.Kick = function() 
            ...
            Notify("Anti Kick", "Off (reload script to restore)")
        end
    end
})
...
Notify("Rix Hub", "✓ v1.1 Loaded — Jump Fixed!")
print("✅ RixHub v1.1 Ready with Luna Interface!")
```